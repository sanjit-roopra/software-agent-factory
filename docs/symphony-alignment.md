...
research-grounded `RepositorySkill` is generated on demand by the configured
Researcher for each eligible post-green polish attempt, bound to the profile's
semantic `dependency_fingerprint`, and reaches only that attempt's Implementer,
Tester and Reviewer. If it cannot be generated or verified, it is dropped with
a recorded warning and the already-green run continues.
...
- **Repository-owned, hot-reloaded workflow configuration.** The factory's
  ...
  `RepositorySkill` it enables is generated by the configured Researcher per
  eligible polish run — not selected from a built-in catalog — and that
  invocation is web-only, with fetches restricted to
  `polish.official_documentation_origins` and the exact
  `polish.practice_reference_urls`. A repository only supplies its own
