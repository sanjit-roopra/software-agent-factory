...
than the Codex app-server protocol. Deterministic factory code owns SDLC
transitions, Git operations, quality gates, and acceptance.
...
Every scheduler cycle must conceptually follow:
...
The local MacBook implementation does not require inbound webhooks.
...
For Git repositories, V1 prefers Git worktrees.
...
For debugging, run artifacts remain.
...
target branch. This process keeps reservations and dependency release
deterministic.
...
A worker can hang without cleanly failing.
...
and no way for a repository to supply skill definitions. Instead, the configured
Researcher generates one bounded, research-grounded `RepositorySkill` for the
whole repository. The skill binds to the profile's semantic
`dependency_fingerprint`. It is stored under `factory.data_dir` outside the
target repository, and later runs reuse it until that fingerprint changes. An
...
The reviewer must use a different model family.
...
These signals must be authoritative where applicable.
...
- **Reconciliation before dispatch.** Every tick reconciles this process's
  ...
  candidates. A manual `factory run` started since the last tick is honored
  immediately.
...
- **Repository-owned, hot-reloaded workflow configuration.** The factory's
  ...
  selected from a built-in catalog). The skill is stored outside the target
  repository and reused across runs until the dependency fingerprint changes.
  That invocation is web-only, with fetches restricted to
...
- **Durable scheduler claims.** Live reservations stay in-memory. The durable
  source of truth combines the persisted `FactoryRun` and workspace `flock`.
  This design is sufficient for one local process.
  It also prevents the addition of a database (`AGENTS.md`: "Keep V1 small").
